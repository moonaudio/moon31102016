...
class RocketWeb_GoogleBaseFeedGenerator_Model_Tools extends Varien_Object
{
    ...
    public function isChildOfProductType($type_id, $sku, $parent_type_id, $store_id = null)
    {
        ...
        elseif (in_array($parent_type_id, array(RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_Subscription_Grouped::PRODUCT_TYPE_SUBSCTIPTION_GROUPED,
    ...
    public function lockDuplicates($row, $productParents)
    {
        ...
        // If product does not belog to a complex product, nothing to lock, just process the product.
        if (!array_filter($productParents)) {
            return $skip;
        }
        ...
        if (!empty($productParents['bundle'])
            ...
            if ($parents) {
                ...
                $generator->log(sprintf("Stand alone SKU %s, ID %d is been omitted because is part of bundle product %s.", $row['sku'], $row['entity_id'], $prods_msg));
                $skip = true;
            }
        }
    ...
    {
        return $this->getFeed()->getConfig('options_mode') == RocketWeb_GoogleBaseFeedGenerator_Model_Source_Product_Option::MULTIPLE_ROWS;
    }
}
