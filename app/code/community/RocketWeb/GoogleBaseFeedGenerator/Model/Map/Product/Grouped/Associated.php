...
class RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_Grouped_Associated extends RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_Abstract
{
    ...
    public function mapColumnLink($params = array())
    {
        ...
        $add_unique = $this->getFeed()->getConfig('grouped_associated_products_link_add_unique');

        switch ($this->getFeed()->getConfig('grouped_associated_products_link')) {
            case RocketWeb_GoogleBaseFeedGenerator_Model_Source_Product_Grouped_Associated_Link::FROM_PARENT:
                $value = $this->getParentMap()->mapColumn('link');
                if ($add_unique) {
                    $value = $this->addUrlUniqueParams($value, $product);
                }
                break;
            case RocketWeb_GoogleBaseFeedGenerator_Model_Source_Product_Grouped_Associated_Link::FROM_ASSOCIATED_PARENT:
