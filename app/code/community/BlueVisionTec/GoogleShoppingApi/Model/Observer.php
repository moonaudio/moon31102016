    ...
    public function checkSynchronizationOperations(Varien_Event_Observer $observer) {
        ...
        if ($flag->isExpired()) {
            Mage::getModel('adminnotification/inbox')->addMajor(
                    Mage::helper('googleshoppingapi')->__('Google Shopping operation has expired.'), Mage::helper('googleshoppingapi')->__('One or more google shopping synchronization operations failed because of timeout.')
            );
            $flag->unlock();
        }
        return $this;
        ...
        foreach ($stores as $_storeId => $_store) {
            ...
            try {
                ...
                $this->_getLogger()->addMajor(
                        Mage::helper('googleshoppingapi')->__('An error has occured while syncing products with google shopping account.'), Mage::helper('googleshoppingapi')->__('One or more products were not synced to google shopping account. Refer to the log file for details.')
                );
                Mage::logException($e);
