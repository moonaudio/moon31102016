...
class MDN_SimpleBarcodeInventory_AdminController extends Mage_Adminhtml_Controller_Action {
    ...
    public function getProductInformationAction() {
        ...
        try {
            ...
            {
                $warehouseId = Mage::helper('SimpleBarcodeInventory')->getWarehouse();
                $warehouse = Mage::getModel('AdvancedStock/Warehouse')->load($warehouseId);

                $stockItem = $warehouse->getProductStockItem($product->getId());
                if (!$stockItem->getId())
                {
                    $stockItem->setQty(0);
                }
    ...
    public function commitProductStockAction() {
        ...
        try {
            ...
            $newStockLevel = $this->getRequest()->getParam('new_stock_value');
            ...
            if (!Mage::getStoreConfig('advancedstock/erp/is_installed')) {
                $product->getStockItem()->setQty($newStockLevel)->save();
            }
            else
            {
                $warehouseId = Mage::helper('SimpleBarcodeInventory')->getWarehouse();
                ...
                if (!$stockItem->getId())
                    $stockItem = mage::getModel('cataloginventory/stock_item')->createStock($productId, $warehouseId);
    ...
    public function changeWarehouseAction()
    {
        try
        {
            ...
            Mage::helper('SimpleBarcodeInventory')->setWarehouse($warehouseId);

            Mage::getSingleton('adminhtml/session')->addSuccess($this->__('Warehouse selected'));
        }
        catch(Exception $ex)
        {
