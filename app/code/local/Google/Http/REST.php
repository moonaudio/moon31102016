...
class Google_Http_REST
{
  ...
  {
    $gcata = json_decode($req->getPostBody());
    
    $gcat = $gcata->googleProductCategory;
    if ($gcat != '') {
            $gcatnew = str_replace("MPPlayers", "MP3 Players", $gcat);
            $gcata->googleProductCategory = $gcatnew;
        }
    $req->setPostBody(json_encode($gcata));
  ...
  {
    mage::log($req,1,'googleapi.log');
    $httpRequest = $client->getIo()->makeRequest($req);
    $httpRequest->setExpectedClass($req->getExpectedClass());
    mage::log($httpRequest,1,'googleapi.log');
