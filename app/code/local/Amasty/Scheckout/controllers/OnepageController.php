...
//                );
        ...
        $billingAddress = $this->getOnepage()->getQuote()->getBillingAddress()->getData();
        $usingCase = isset($billingAddress['use_for_shipping']) ? (int) $billingAddress['use_for_shipping'] : 0;
        if($usingCase){
            $result = $this->validateCountryRestrict($billingAddress);
        } else {
            $result = $this->validateCountryRestrict($this->getOnepage()->getQuote()->getShippingAddress()->getData());
        }
        
            if($result['error'] == 1){
                $messagesBlock = $this->getLayout()->getMessagesBlock();
                $messagesBlock->addError($result['message']);
                $res = array(
                "status" => "error",
                "errorsHtml" => $messagesBlock->toHtml(),
                "errors" => implode("\n", $result['message'])
