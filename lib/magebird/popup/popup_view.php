...
class popup_view{
  ...
  public function toHtml($collection,$helper,$currentProduct)
  {  
    $this->helper = $helper;    
    $style = "";
    $script = "";
    ...
    foreach($collection as $popup): 
      ...
      if($popup['popup_type']==1){  
        if($popup['image'] && $popup['width_unit']==2){
          $imgSrc = $this->helper->getParam('baseUrl') . "media/" . $popup['image'];          
        }elseif($popup['image']){
          $imgSrc = $this->helper->getParam('baseUrl') . "media/" . $popup['image'];          
        } 
  ...
  public function parseProdAttr($content,$product){  
